...
export function SpokenPlayer({ content, asset, variants=[], playbackRate, transcript, transcriptSegments=[], chapters=[], initialPositionMs=0, onProgress, onEnded, onReportIssue, onClose }: Props) {
  ...
  const activeMedia=mediaKind==="VIDEO"?videoRef.current:audioRef.current;
  ...
  const audioAvailable=asset.kind==="AUDIO"||variants.some((item)=>item.kind==="AUDIO");
  const videoAvailable=asset.kind==="VIDEO"||variants.some((item)=>item.kind==="VIDEO");
  ...
  return <section className="spoken-player" aria-label="Lecteur de contenu parlé">
    <audio ref={audioRef} style={{display:mediaKind==="AUDIO"?"block":"none"}} src={asset.url} preload="metadata" onLoadedMetadata={(e)=>{const a=e.currentTarget;a.currentTime=Math.min(initialPositionMs/1000,a.duration||0);setDuration(a.duration||0);setLoading(false);}} onTimeUpdate={(e)=>{
