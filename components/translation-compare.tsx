...
export function TranslationCompare({ primary, alternatives }: Props) {
  ...
  if(!alternatives.length) return <small className="translation-credit">Traduction · {primary.author}</small>;
  return <div className="translation-compare">
    ...
    {open && <div className="translation-options">{[primary,...alternatives].map((item)=><article key={item.id}><p>{item.text}</p><footer><strong>{item.name}</strong><span>{item.author}</span>{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer">Source</a>}</footer></article>)}</div>}
  </div>;
}
