...
import { ChevronDown, ChevronUp, ListMusic, Play } from "lucide-react";
...
export function PlaybackQueue({ queue, onPlay, onRemove, onMove, onClear, currentId }: Props) {
  return <section className="playback-queue" aria-label="File d’attente">
    ...
    {queue.length === 0 ? <div className="empty-library compact"><ListMusic size={20}/><strong>File vide</strong><p>Ajoutez plusieurs écoutes pour les préparer à l’avance.</p></div> :
      <>
      <div className="queue-list">{queue.map((entry,index)=><div className={`queue-row ${currentId === entry.id ? "current" : ""}`} key={entry.id}>
        <button type="button" className="queue-main" onClick={()=>onPlay(entry)} aria-current={currentId === entry.id ? "true" : undefined}><span>{currentId === entry.id ? <Play size={13} fill="currentColor"/> : String(index+1).padStart(2,"0")}</span><div><strong>{entry.item.title}</strong>{entry.item.subtitle && <small>{entry.item.subtitle}</small>}</div></button>
