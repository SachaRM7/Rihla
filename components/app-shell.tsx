          ...
          {activeView === "search" && (
            <div className="content-stack">
              ...
              {searchType === "spoken" && hasSpokenContents && !selectedCreator && !selectedSeries && <div className="spoken-catalog-list">{spokenContents.map((item) => {
                ...
                return <div className="spoken-catalog-row" key={item.id}><button type="button" onClick={() => playSpokenContent(item)}><div><strong>{item.title}</strong><small>{item.type.replaceAll("_"," ").toLocaleLowerCase("fr")}</small></div><ChevronRight size={18}/></button>{asset && <OfflineDownloadControl allowed={allowed} status={downloadStates[item.id]} onDownload={()=>{ if(library.wifiOnlyDownloads){ const connection=(navigator as Navigator & {connection?:{type?:string}}).connection; if(connection?.type && connection.type!=="wifi"){setShareMessage("Téléchargement réservé au Wi-Fi");return;} } setDownloadStates((current)=>({...current,[item.id]:"QUEUED"})); setShareMessage("Téléchargement prêt · stockage hors connexion à connecter"); }} />}</div>;
              })}</div>}
